...
class MatchNet(nn.Module):
    ...
    def forward(self, x, samples):
        ...
        :param x:
        :param samples: uniformly sampled cuboids in [-1,1], shape torch.Size([bs, bins**3, nSamples, 3])
        :return:
        """
        nSamples = samples.shape[2]
        x, _ = self.encoder(x)  # bs x 11*bins**3
        z, q, t, p = torch.split_with_sizes(x,
                                            tuple(torch.tensor([3, 4, 3, 1]) * self.bins ** 3), axis=1)
        z = z.reshape(z.shape[0], -1, 3).unsqueeze(2).repeat(1, 1, nSamples, 1) * 0.5 # bs x bins^3 x nSaples x 3
        t = t.reshape(t.shape[0], -1, 3).unsqueeze(2).repeat(1, 1, nSamples, 1) * 0.5 # bs x bins^3 x nSaples x 3
...
if __name__ == '__main__':
    ...
    mn = MatchNet(bins=bins_per_face)
    samples = sample_partial_cuboid(batch_size, bins_per_face, samples_per_face)
    out = mn(sim_data, samples)


    import numpy.random as rm
    from utils.visualization import plot_pc_mayavi
    c = [tuple(rm.random(3)) for i in range(ncuboid)]
    plot_pc_mayavi([samples[0][i] for i in range(ncuboid)], colors=c)
